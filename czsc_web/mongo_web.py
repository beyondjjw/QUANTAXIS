...
import czsc
from localdatas.MongoHLOC import MongoHLOC as mongodata
...
ts.set_token("26389762c8f7fa9640cb7990354e8fa848bc97ea0f76756300e1cc79")
...
def get_kline(code, end_date, freq='30min', asset='E'):
    """获取指定级别的前复权K线
    ...
    try:
        df = mongodata().get_data_by_day(code, start_date, end_date)
    ...
    df['symbol'] = df.code
    k = df[['code', 'symbol','date', 'open', 'close', 'high', 'low', 'volume']]
...
# 端口固定为 8005，不可以调整
define('port', type=int, default=7733, help='服务器端口')
...
class KlineHandler(BaseHandler):
    ...
    def get(self):
        ...
        kline = get_kline(code=ts_code, end_date=trade_date, freq=freq, asset=asset)
        # kline.loc[:, "date"] = pd.to_datetime(kline.dt)
        print(kline.head())
        ...
        self.finish({'kdata': ka.values.tolist()})
...
# http://localhost:7733/?ts_code=000001.SH&asset=I&trade_date=20200613&freqs=D,30min,5min,1min
# http://localhost:7733/kline?ts_code=000001&asset=I&trade_date=2020-06-13&freq=D
# http://localhost:7733/?ts_code=000001&asset=I&trade_date=2020-06-13&freq=D
